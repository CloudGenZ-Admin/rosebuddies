...
const Polaroid = ({ src, alt, baseRotation, positionClasses, mouseX, mouseY, depth, tapeColor = "bg-brand-peach/60", children }) => {
  const moveX = mouseX * depth;
  const moveY = mouseY * depth;

  return (
    <div 
      className={`absolute group bg-brand-pink-light border-2 border-brand-pink-dark p-3 pb-12 shadow-2xl rounded-2xl cursor-pointer w-48 md:w-64 ${positionClasses}`}
      style={{
    ...
    >
      ...
      <div className="w-full h-48 md:h-56 bg-brand-pink-dark/40 rounded-lg overflow-hidden relative transition-transform duration-500 group-hover:scale-[1.03]">
...
export default function Hero() {
  ...
  return (
    <section 
      className="relative min-h-screen flex items-center pt-24 pb-32 overflow-hidden px-6 md:px-12 lg:px-24 bg-brand-pink"
      style={{ 
        backgroundImage: `radial-gradient(var(--color-brand-pink-dark) 2px, transparent 2px)`, 
        backgroundSize: '32px 32px' 
      }}
    >
      <div className="absolute top-[-10%] right-[-5%] w-[600px] h-[600px] bg-brand-primary/10 rounded-full blur-[100px] animate-float z-0 pointer-events-none"></div>
      <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] bg-brand-accent/10 rounded-full blur-[120px] animate-float z-0 pointer-events-none" style={{ animationDelay: '2s' }}></div>

      <FloatingDoodle classes="top-[15%] left-[5%] text-brand-accent w-12 h-12 opacity-70" delay="0s">
      ...
      <FloatingDoodle classes="bottom-[10%] left-[45%] text-brand-secondary w-16 h-16 opacity-60" delay="1s">
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center w-full relative z-10">
        ...
        <div className="max-w-xl relative z-20">
          <div className="relative inline-block mb-8">
            <div className="absolute -top-2 -left-4 w-10 h-4 bg-brand-secondary/40 -rotate-12 z-50 shadow-sm backdrop-blur-sm"></div>
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-brand-pink-light border-2 border-brand-pink-dark text-brand-dark font-bold font-sans text-sm shadow-sm relative z-10 hover:-rotate-2 transition-transform cursor-default">
              <span className="w-2 h-2 rounded-full bg-brand-primary animate-pulse"></span>
              Real People. Real Conversations.
            </div>
          </div>

          <h1 className="text-5xl md:text-7xl font-serif text-brand-dark leading-[1.1] mb-6 relative">
            Find Your <br/>
          ...
          <p className="text-2xl font-serif font-medium text-brand-secondary mb-5 leading-snug">
            Real People. Real Conversations. Real Friendships.
          </p>
          
          <p className="text-lg text-brand-dark/80 mb-10 font-sans leading-relaxed font-semibold">
          ...
          <div className="relative flex flex-col sm:flex-row items-center gap-6 z-50 mt-4">
            <div className="absolute -top-12 left-10 md:-left-12 md:top-2 text-brand-primary w-12 h-12 animate-bounce rotate-[15deg] md:-rotate-[70deg] z-50 pointer-events-none">
              <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round">
                <path d="M10 50 Q 50 10 90 50 M 70 30 L 90 50 L 70 70" />
              </svg>
            </div>

            <a href="#interest" className="w-full sm:w-auto flex items-center justify-center gap-3 bg-brand-dark text-brand-pink-light font-bold text-lg px-8 py-4 rounded-full shadow-[6px_6px_0px] shadow-brand-primary hover:shadow-[8px_8px_0px] hover:shadow-brand-accent hover:-translate-y-1 hover:bg-black transition-all duration-300 group">
              [ Find My Circle ]
              <ArrowRight className="group-hover:translate-x-1 transition-transform" size={20} />
            </a>
            <a href="#meet" className="w-full sm:w-auto text-center px-8 py-4 border-4 border-brand-dark rounded-full font-bold text-brand-dark bg-brand-yellow shadow-[4px_4px_0px_#2A2A2A] hover:translate-y-1 hover:shadow-none transition-all duration-300">
        ...
        <div className="relative h-[600px] hidden lg:block perspective-[1000px] z-10 pointer-events-none lg:pointer-events-auto">
          ...
          <Polaroid 
            ...
            mouseY={mousePos.y}
            depth={10}
            tapeColor="bg-brand-secondary/50" 
          >
