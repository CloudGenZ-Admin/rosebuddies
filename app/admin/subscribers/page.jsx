...
export default function SubscribersPage() {
  ...
  // Fetch data from API, reacting to the debounced search query
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const token = localStorage.getItem("adminToken");
      try {
        // Dynamically add search query to URL if it exists
        const url = debouncedQuery 
          ? `/api/admin/footer-subscribers?search=${encodeURIComponent(debouncedQuery)}`
          : "/api/admin/footer-subscribers";
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 md:mb-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm sm:text-base text-gray-500">
            Users who subscribed from the footer.
          </p>
        </div>
